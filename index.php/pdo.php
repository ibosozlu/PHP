...
quand il n'y a plus de résultat, ça retourne false
*/
...
var_dump($result); //  le nombre de lignes modifiés
// syntaxe heredoc
/*
$query = <<<EOS
INSERT INTO employe (
...
EOS;
$pdo->exec($query);
echo $pdo->lastInsertId(); // dernier identifiant inséré
*/
?>
<h2>Select & fetchColumn</h2>
<?php
$stmt = $pdo->query('SELECT count(*) AS nb FROM employe');
var_dump($stmt->fetchColumn()); // premier résultat de la première ligne
?>
<h2>Quote</h2>
<?php
$nom = "D'artagnan";
// erreur de syntaxe à cause de a quote
...
$stmt->bindValue(1, 'informatique'); // donne une valeur au premier point d'interrogation dans la requête
...
echo '</pre>';
?>
<h2>Requêtes préparées (paramètres nommés)</h2>
<?php
...
$stmt = $pdo->prepare($query);
// si la valeur est nulle, il faut le préciser en troisième paramètre
